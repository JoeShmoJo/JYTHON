...
and writes into that folder:
    ...
    <check>_summary.csv    one row per reservoir or diversion
    plots/<check>.html     interactive plots; pick an element from the dropdown,
...
Diversions   Did DiversionFromCSV ask for the config value, and was it diverted?
...
def checkFiro(groups, configFile, dates):
    """One summary row and one set of plot traces per reservoir in the config."""
    rows, plots = [], {}
    for key, (name, cfg) in sorted(readDailyConfig(configFile).items()):
        ...
        inflow = col(groups, "reservoirs", "%s-Pool Flow-IN" % name).reindex(dates)
...
def checkMinFlow(groups, minFlowFile, withdrawalFile, dates):
    ...
    for key in sorted(set(minFlow) | set(withdrawal)):
        ...
        plots[name] = [
            ("Outflow at %s" % releasedAt, out, "y1", {"color": "#2ca02c"}),
            ("Min flow + withdrawal (config)", required, "y1", {"color": "#d62728", "dash": "dash"}),
            ("Short, unexplained", out.where(short & ~maxBelow), "y1",
             {"mode": "markers", "color": "#ff7f0e"}),
            ("Min limit", minLim, "y1", {"color": "#9467bd", "dash": "dot"}),
            ("Max limit", maxLim.where(maxLim < NO_LIMIT_CFS), "y1",
             {"color": "#8c564b", "dash": "dot"}),
        ]
        if ruleVal is not None:
            plots[name].append(("%s rule value" % MINFLOW_RULE_NAME, ruleVal.reindex(dates), "y1",
                                {"color": "#e377c2", "width": 1}))
        for c in other:
            plots[name].append((c, groups["rules"][c].reindex(dates), "y1",
                                {"color": "#bcbd22", "width": 1}))
    return pd.DataFrame(rows), plots
...
def checkDiversions(groups, configFile, dates):
    ...
    for key, (name, cfg) in readDailyConfig(configFile).items():
        ...
        actual = col(groups, "diversions", "%s Flow-DECISION" % name)
        if actual is None and spec is None:
            rows.append({"element": name, "in_results": "no"})
            continue
        row = {"element": name, "in_results": "yes"}
        if spec is not None:
            spec = spec.reindex(dates)
            ok = spec.notna()
            row["rule_matches_config_pct"] = _pct(
                (ok & ((spec - expected).abs() <= RULE_TOL_CFS)).sum(), ok.sum())
        if actual is not None:
            actual = actual.reindex(dates)
            ok = actual.notna()
            row["config_af"] = round(expected[ok].sum() * CFS_DAY_TO_AF, 0)
            row["diverted_af"] = round(actual[ok].sum() * CFS_DAY_TO_AF, 0)
            # Returns are negative, so compare sizes
            short = (expected.abs() - actual.abs()).clip(lower=0)[ok]
            row["days_short"] = int((short > RULE_TOL_CFS).sum())
            row["shortfall_af"] = round(short.sum() * CFS_DAY_TO_AF, 0)
        rows.append(row)
        plots[name] = [
            ("Config", expected, "y1", {"color": "#d62728", "dash": "dash"}),
            ("Rule asked for (SPEC)", spec, "y1", {"color": "#e377c2", "width": 1}),
            ("Diverted (DECISION)", actual, "y1", {"color": "#1f77b4"}),
        ]
...
def writeReport(runDir, runInfo, configRoot, configFiles, results):
    ...
    parts.append("</ul>")
    for check, (table, plotFile, note) in results.items():
        parts.append("<h2>%s</h2><p>%s</p>" % (check, note))
        if plotFile:
            parts.append("<p><a href='plots/%s'>Open plots</a></p>" % plotFile)
...
def main():
    ...
    checks = [
        ...
        ("Diversions", ["Diversions"], lambda: checkDiversions(groups, files["Diversions"], dates),
         "Flow (cfs)", None,
         "rule_matches_config_pct: DiversionFromCSV asked for the config value. "
         "days_short / shortfall_af: diverted less than the config (returns compared by size)."),
    ]
    results = {}
    for name, needs, run, y1, y2, note in checks:
        ...
        plotFile = None
        if plots:
            plotFile = "%s.html" % name
            dropdownFigure(name, plots, y1, y2).write_html(
                os.path.join(plotDir, plotFile), include_plotlyjs="directory")
        results[name] = (table, plotFile, note)
        print("\n== %s ==" % name)
        with pd.option_context("display.width", 200, "display.max_columns", 30):
            print(table.to_string(index=False) if len(table) else "nothing to check")

    writeReport(runDir, runInfo, configRoot, files, results)
