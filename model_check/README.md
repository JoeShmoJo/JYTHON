...
| File | Contents |
|---|---|
| `reservoirs.csv` | inflow, outflow, elevation per reservoir |
...
| `plots/<check>.html` | interactive plots: pick an element from the dropdown, click the legend to hide or show series |
...
- *Achieving:* is the pool within `ELEV_TOL_FT` of the target? A day off
  ...
  markers on the plot. That is where to look.
...
**Diversions.** Whether DiversionFromCSV asked for the config value, and
whether it was diverted.
