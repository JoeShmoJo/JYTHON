...
**What it is:** HEC-ResSim Jython scripts for a Willamette basin model, plus the
tooling that keeps them honest. `data/scripts/` is copied wholesale into a ResSim
watershed to deploy.

**Writing any code under `data/scripts/`?** Read `data/scripts/CLAUDE.md` first.
It is Jython 2.7 running inside ResSim, not ordinary Python, and it cannot be run
or tested the ordinary way.

**How to test it:** the parts that are ours run on desktop Python with the Java
imports stubbed.

```powershell
python data\scripts\externalRules\_offline_tests\test_MinFlowPlusWithdrawal.py
python data\scripts\externalRules\_offline_tests\test_DiversionFromCSV.py
```

Those cover CSV parsing and arithmetic. They cover nothing about ResSim itself.
The real test is running the model.

**How to check imports against a ResSim install:**
...
**Generated files. Never edit by hand, rerun the generator.**
...
**Large directories that will blow up a diff or a context window:**

- `data/javaDocs/` — 1187 files, about 621,000 lines. Never open or browse it.
  Grep `data/scripts/_reference/ressim_api.txt` instead.
- `data/daily_diversions_returns_ALT.csv` — 31,414 rows.

**Jython writes `*$py.class` next to every module it imports.** They are
