...
def initRuleScript(currentRule, network):
    ...
    for resvName in ["Hills Creek", "Lookout Point"]:
        ...
        maxConStor = elevStorTable.interpolate(maxConElev[resvName])
        conStor = maxConStor - minConStor
...
def runRuleScript(currentRule, network, currentRuntimestep):
    ...
    #Apply limits
    ruleValue = max(0, min(HIGHEST_MIN_FLOW, min(max(MIN_FLOW_LOW_LIMIT,inflow+RELEASE_ABOVE_INFLOW), release)))
