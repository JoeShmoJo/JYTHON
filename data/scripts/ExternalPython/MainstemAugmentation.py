...
CUR_DIR = Path(__file__).resolve().parent          # ...\scripts\ExternalPython
SCRIPTS_DIR = CUR_DIR.parent                      # ...\scripts

# PKL now lives in ExternalPython
PKL_PATH = CUR_DIR / "median_remaining_by_day.pkl"

# CSV config files live in ...\scripts\externalSVs
EXTERNAL_SV_DIR = SCRIPTS_DIR / "externalSVs"
...
def readallpaths_dpart(dss_file: str, base_path_dict: dict, dparts: list, fid=None) -> pd.DataFrame:
    """
    Read all paths for every D-part in `dparts` through a single HecDss
    session, then splice each (alias, dpart) pair back to a single alias
    column.

    Steps:
      1. Build combined path dict with keys like alias__{dpart}
      2. Read all paths in one session (reusing fid if provided)
    ...
    try:
        for alias, path in combined_path_dict.items():
            try:
                ...
                print(f"[DSS] Failed '{alias}' -> {path} in {os.path.basename(dss_file)}: {e}")
    finally:
        if _opened_here:
            fid.close()
...
try:
    ...
    for fpart in fparts:
        ...
        # *** ONLY CHANGE IN THE LOOP: use readallpaths_dpart instead of readallpaths ***
        data = readallpaths_dpart(DSS_FILE_IN, filtered_path_dict, DPART_STARTS, fid=fid_in)
