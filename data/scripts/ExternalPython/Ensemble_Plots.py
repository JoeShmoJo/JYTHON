...
def splice_dpart_columns(
    ...
    for fpart in fparts:
        ...
        # dropna() first so that NaN-padded rows from the shared DataFrame index
        # don't mask real values from the other D-part chunk when deduplicating.
        target_col = f"{base_alias}__{fpart}"
        pieces = [df[c].dropna().rename(target_col) for c in dpart_cols]
...
def _xaxis_range(ens_cols, obs_series=None):
    """
    Return [x_start, x_end] where:
      x_start = earliest non-NaN timestamp in obs_series (if any), else
                earliest non-NaN timestamp across ens_cols in data_ens.
      x_end   = latest  non-NaN timestamp across ens_cols in data_ens.

    Either bound falls back to the full data_ens index if nothing better
    is found.
    """
    # --- x_end: last date with real ensemble data ---
    ens_data = data_ens[ens_cols].apply(pd.to_numeric, errors="coerce")
    valid_mask = ens_data.notna().any(axis=1)
    x_end = ens_data.index[valid_mask].max() if valid_mask.any() else data_ens.index[-1]

    # --- x_start: first date with real observed data (if supplied) ---
    if obs_series is not None and not obs_series.empty:
        x_start = obs_series.index.min()
    else:
        x_start = ens_data.index[valid_mask].min() if valid_mask.any() else data_ens.index[0]
...
for res_abbr, res_name in RESERVOIR_NAME_MAP.items():
    ...
    if elev_cols:
        ...
        # Use all ensemble elev cols (base + synthetic) for x_end.
        elev_cols_all = _get_ensemble_cols(data_ens, f"{res_abbr}_elev__", ensemble_fparts_model)
        xrange_elev = _xaxis_range(elev_cols_all, obs_s if not obs_s.empty else None)
    ...
    if out_cols:
        ...
        obs_out_s = pd.Series(dtype=float)
        ...
        # Use all ensemble outflow cols (base + synthetic) for x_end so the
        # right edge reaches the end of the synthetic traces, not just base.
        out_cols_all = _get_ensemble_cols(data_ens, f"{res_abbr}_outflow__", ensemble_fparts_model)
        xrange_out = _xaxis_range(out_cols_all, obs_out_s if not obs_out_s.empty else None)
