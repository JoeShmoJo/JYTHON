...
dataQueue     = None
#=========================================================================
class Logger(java.io.PrintStream) :
	...
	def __init__(self, filename) :
		...
		fos = java.io.FileOutputStream(filename, True)
		java.io.PrintStream.__init__(self, fos)
		self.oldOut = java.lang.System.out
		self.oldErr = java.lang.System.err
		self.filename = filename
		self._cons = java.lang.System.console()
		self._file = java.io.PrintStream(fos)
		java.lang.System.setOut(self)
		java.lang.System.setErr(self)
	...
	def close(self) :
		...
		java.lang.System.setOut(self.oldOut)
		java.lang.System.setErr(self.oldErr)
		self._can_output = False
	...
	def print_(self, arg) :
		...
		if self._cons : 
			...
			self._cons.flush()
		if self._file : self._file.print(arg)
	...
	def write(self, *args) :
		...
		if len(args) == 1 :
			...
			arg = java.lang.String(buf, offset, length)
			if self._cons : 
				self._cons.printf("%s", [str(arg)])
				self._cons.flush()
...
def getLogfileName(id=None, directory=None) :
	...
	if directory is None : directory = outputDir
...
def log(string, newLine = True) :
	...
	if newLine : java.lang.System.out.println(string)
	else       : java.lang.System.out.print_(string)
#=========================================================================
...
class LogPanel(JFrame) :
	def __init__(self, title) :
		...
		self.textSize = 0
		self.dataQueue = java.util.concurrent.LinkedBlockingQueue()
		self.statusQueue = java.util.concurrent.LinkedBlockingQueue()
...
def runSimulations(runPaths, separateLogFiles) :
	...
	for runPath in runPaths :
		...
		if logPanel :
			if trialName :
				logPanel.setStatus("Computing simulation %d of %d: %s, alternative %s, trial %s" % \
				                   (i, count, simulationName, alternativeName, trialName))
			else :
				logPanel.setStatus("Computing simulation %d of %d: %s, alternative %s" % \
				                   (i, count, simulationName, alternativeName))
		log("")
		log("###")
		if trialName :
			log("### Computing simulation %s, alternative %s, trial %s" % \
			    (simulationName, alternativeName, trialName))
		else :
			log("### Computing simulation %s, alternative %s" % \
			    (simulationName, alternativeName))
		log("###")
		if separateLogFiles :
			if trialName :
		...
		if trialName :
			for trial in simRun.getTrials() :
				...
				raise Exception("Cannot find trial %s in alternative %s in simulation %s" % \
				                 (trialName, alternativeName, simulationName))
